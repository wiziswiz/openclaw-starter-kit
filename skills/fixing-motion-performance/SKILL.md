...
description: >
  Fix animation performance issues by applying prioritized rules for UI animations,
  transitions, and motion effects to prevent janky interactions and rendering bottlenecks.
  
  Use when:
  - User asks to review animations, motion, or transitions for performance
  - Code has janky scrolling, animation, or interaction issues
  - Implementing new UI animations or motion effects
  - User mentions "/fixing-motion-performance" or animation optimization
  - Working with CSS animations, JavaScript motion libraries, or scroll effects
  - Need to audit will-change, transforms, filters, or layout-triggering animations

  Don't use when:
  - User wants to add new animations (not fix existing ones)
  - Working on static UI without motion or animations
  - Backend performance issues not related to frontend rendering
  - General code review not focused on animation performance
  - Animation libraries need migration (unless explicitly requested)
  - Non-performance animation questions (timing, easing, design)
  
  Inputs: Files with animations, CSS transitions, or JavaScript motion code
  Outputs: Specific violations with code snippets and performance-optimized alternatives
  Success: Concrete fixes that eliminate layout thrashing and improve animation smoothness
---
