...
description: >
  Ship correct, complete web metadata including SEO tags, Open Graph cards, 
  canonical URLs, structured data, and social sharing optimization.
  
  Use when:
  - User asks about SEO, metadata, or social sharing optimization
  - Working on page titles, descriptions, Open Graph, or Twitter cards
  - Need to fix duplicate or missing meta tags
  - User says "/fixing-metadata" or requests metadata review
  - Building new pages that need proper SEO and social metadata  
  - Issues with search engine indexing or social card previews

  Don't use when:
  - User wants backend SEO or server configuration changes
  - Working on non-web platforms (mobile apps, desktop apps)
  - Content strategy or copywriting not related to technical metadata
  - User requests new SEO frameworks (unless explicitly asked)
  - General web development not focused on metadata issues
  - Analytics or tracking setup (different from metadata)
  
  Inputs: HTML files, React components, or pages with metadata concerns  
  Outputs: Specific metadata violations with exact tag fixes and explanations
  Success: Complete, non-duplicated metadata that works for SEO and social sharing
---
